...
def getprop(obj, path, key_error_as_none=False):
    ...
    if pp not in obj:
        if not key_error_as_none:
            raise KeyError('Path not found in object: %s (%s)' % (path, pp))
        else:
            return None

    return getprop(obj[pp], pn, key_error_as_none)


def setprop(obj, path, val, key_error_as_none=False):
    """
    ...
    return setprop(obj[pp], pn, val, key_error_as_none)


def delprop(obj, path, key_error_as_none=False):
    """
    ...
    return delprop(obj[pp], pn, key_error_as_none=False)
...
def exists(obj, path):
    ...
    found = False
    try:
        found = getprop(obj, path, key_error_as_none=False) is not None
    except KeyError:
        pass
    except TypeError:
        pass
...
def test_get():
    assert getprop(
        TEST_OBJ, 
        PATH_DELIM.join(('person', 'name'))) == TEST_OBJ['person']['name']
    assert getprop(
        TEST_OBJ, 
        PATH_DELIM.join(('person', 'age'))) == TEST_OBJ['person']['age']
